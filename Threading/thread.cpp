...
std::mutex mtx;
void sayhello() { std::cout << "hello from new thread!" << std::endl; }
...
int main() {
  ...
  // t4 is going to read and all thread are independent they dont have to be
  // dependent on each other in complied language;
  std::thread t4(task4);
  std::thread t5(task4);
